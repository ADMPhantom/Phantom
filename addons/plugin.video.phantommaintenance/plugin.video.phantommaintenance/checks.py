...
def checkJava(dp):
    ...
    dialog.ok("Phantom Dev", "You Dont Have Java Installed We Need To Download", "You Will Only Need To Do This Once")
    ...
    if not extract.all(lib, JAVAPATH, dp):
        ...
        dialog.ok('XBMCHUB Maintenance','There was a problem reading the Java file','','')
        return False
...
def remove(filename, DEBUG = False):
    ...
    try:
        if not makeExec(os.path.join(ADDONPATH, 'signer', 'testkey.pk8')):
            d.ok('XBMCHUB Maintenance','There was a problem setting permissions', 'on testkey.pk8','')
            return False
    except Exception, e:
    ...
    try:
        ...
        print str(e)
        d.ok('XBMCHUB Maintenance','There was a problem reading the file','','')
        return cleanUp(imgPath, False, DEBUG)
    ...
    if not newScript:
        ...
        d.ok('XBMCHUB Maintenance','There were no checks to remove','','')
        zin.close()
        try:
            os.remove(dest)
        except:
    ...
    if good: 
        ...
        d.ok('XBMC Maintenance', 'Signed file created sucessfully', '', '')
    else:
        print "**** remove: CRAP SIGNED FILE DOES NOT EXISTS"
        d.ok('XBMC Maintenance','Failed to sign file', 'Check Logs', '')
